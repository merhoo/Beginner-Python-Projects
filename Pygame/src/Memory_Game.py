...
class CardConfig(object): # managing all the cards
    def __init__(self, rows, cols):
        ...
        self.image_width = self.cardset[0].rect.width # width of one card
        self.image_height = self.cardset[0].rect.height # height of one card
        self.top_margin = 30 #making space for time and score
        self.padding = 10 # space between cards
    ...
    def reset(self):
        self.cards = [] # ????
        random.shuffle(self.card_indexes) # why do you need to do this?
        for i in range(self.total_pairs):# creates two cards
            self.cards.append(self.cardset[i].clone())
            self.cards.append(self.cardset[i].clone())
        random.shuffle(self.cards) # shuffle all of the cards created
    ...
    def get_index_of_clicked_card(self, x, y):
        ...
        for card in self.cards:
            if self.rect[0] < x < self.rect[0] + self.image_width:
                if self.rect[1] + self.image_height < y < self.rect[1]:
                    return card.index
                return -1
            return -1
    ...
    def is_match(self):
        ...
        if self.flipped[0] and self.flipped[1] != -1:
...
class Card(pygame.sprite.Sprite):
    ...
    def __init__(self, image_number):
        ...
        filename = "images/monster{0:d}.bmp".format(image_number)
    ...
    def contains(self, x, y):
        ...
        if self.status != Card.HIDDEN:
            if self.rect[0] < self.x < self.rect[0] + self.image_width:
                if self.rect[1] - self.image_height < self.y < self.rect[1]:
                    return self.rect
                return False
            return False
    ...
    def paint(self, screen):
        ...
        hidden.''' 
        if self.status == Card.BACK_SHOWN:
            screen.blit(Card.back_image, (self.rect[0],self.rect[1]))
        elif self.status == Card.IMAGE_SHOWN:
...
class Game(object):
    ...
    def __init__(self, rows=3, cols=6):
        product = rows * cols
        if product % 2 != 0:
            raise ValueError("Product of rows and columns must be divisble by 2.")
        if product > NUM_CARDS * 2:
            raise ValueError("Product of rows and columns must not exceed {0:d}.".format(NUM_CARDS * 2))
        self.rows = rows
        self.cols = cols
        self.card_config = CardConfig(self.rows, self.cols) # card manager
    ...
    def reset(self): # reinitializing all variables such as time or score to default once the game is over
        self.card_config.reset() # reshuffle cards
        ...
        global score # have to say global first
        score = 0
...
if __name__ == '__main__': # rows and columns selection
    argc = len(sys.argv)
